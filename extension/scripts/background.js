        ...
        .then(() => {
          ...
          setTimeout(() => {
            chrome.tabs.sendMessage(tab.id, { action: 'toggle_sidebar' })
              .catch(error => console.error('Error sending message after injection:', error));
            isProcessingClick = false;
          }, 500);
        })
        .catch(error => {
...
async function extractContentFromActiveTab() {
  ...
  if (activeExtraction) {
    console.log('Content extraction already in progress, returning existing promise');
  ...
  try {
    ...
    activeExtraction = (async () => {
      ...
      try {
        ...
        if (!activeTab.url) {
          ...
          return { success: false, error: 'Active tab has no URL or is inaccessible' };
        }
        ...
        try {
          if (activeTab.url.startsWith('chrome://') || 
            ...
            return { 
              success: false, 
              error: 'Cannot extract content from this page type. Try on a regular webpage.' 
            };
          }
        } catch (urlError) {
          console.error('Error checking tab URL:', urlError);
          return { success: false, error: 'Error checking tab URL: ' + urlError.message };
        }
        ...
        try {
          return await new Promise((resolve) => {
            chrome.tabs.sendMessage(activeTab.id, { action: 'extract_content' }, (response) => {
              if (chrome.runtime.lastError) {
                console.log('Content script not yet injected, injecting now for extraction');
                ...
                .then(() => {
                  console.log('ContentExtractor.js injected successfully for extraction');
                  return chrome.scripting.executeScript({
                    target: { tabId: activeTab.id },
                    function: () => {
                      if (typeof window.extractPageContent === 'function') {
                        ...
                        console.error('extractPageContent function not found');
                ...
                .then((results) => {
                  if (results && results[0] && results[0].result) {
                    ...
                    resolve({ 
                      success: false, 
                      error: 'Failed to extract content after script injection' 
                    });
                  }
                })
                .catch(error => {
                  console.error('Error in content extraction process:', error);
                  resolve({ 
                    success: false, 
                    error: 'Error injecting content extractor: ' + error.message 
                  });
                });
                ...
                resolve({ 
                  success: false, 
                  error: response?.error || 'Unknown error extracting content' 
                });
              }
            });
          });
          ...
          return { success: false, error: 'Error during content extraction: ' + error.message };
        }
      } catch (error) {
        console.error('Error in extractContentFromActiveTab:', error);
        return { success: false, error: 'Error getting active tab: ' + error.message };
      } finally {
    ...
    return { success: false, error: 'Critical extraction error: ' + error.message };
  }
}
  ...
  if (message.action === 'extract_page_content') {
    ...
    try {
      chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
        if (!tabs || tabs.length === 0) {
          ...
          sendResponse({ 
            ...
            content: { 
              title: 'No Active Tab',
              url: '',
              content: 'Unable to extract content because no active tab was found. Please try refreshing the page or opening the extension on a different page.',
        ...
        extractContentFromActiveTab()
          .then(result => {
            console.log('CocBot: Content extraction result:', 
            ...
            if (!result.success && tabs[0]) {
              result.content = {
                ...
                content: 'Content extraction failed: ' + (result.error || 'Unknown error'),
                timestamp: new Date().toISOString(),
                extractionSuccess: false
              };
            }
          ...
          .catch(error => {
            ...
            sendResponse({ 
              ...
              content: { 
                ...
                content: 'An error occurred while extracting content from this page: ' + error.message,
                timestamp: new Date().toISOString(),
                extractionSuccess: false
              }
            });
      ...
      console.error('CocBot: Critical error in extract_page_content handler:', error);
      sendResponse({ 
        success: false, 
        ...
        content: { 
          title: 'Error Page',
          url: '',
          content: 'A critical error occurred in the content extraction system: ' + error.message,
          timestamp: new Date().toISOString(),
          extractionSuccess: false
        }
      });
