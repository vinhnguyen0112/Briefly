...
import { addMessageToChat, addTypingIndicator, removeTypingIndicator } from './ui-handler.js';
...
export async function processUserQuery(query) {
  ...
  try {
    ...
    if (!apiKey) {
      ...
      addMessageToChat("Hey, I need your OpenAI API key to work. Add it in settings.", "assistant");
    ...
    const messages = await constructPromptWithPageContent(query, state.pageContent, state.history, state.currentConfig);
...
${styleInstructions}`
  ...
  if (pageContent) {
    if (pageContent.extractionSuccess === false) {
      contextMessage.content += "Note: Limited page content extracted. I'll work with what's available.\n\n";
    }
    ...
    if (pageContent.content) {
      ...
      contextMessage.content += "The page content extraction failed. I have limited context about this page.";
    }
  } else {
    contextMessage.content += "No page content was provided. I don't have specific context about what you're viewing.";
  }
...
${pageContent.content.substring(0, 3000)}`;
    ...
    if (data.choices && data.choices.length > 0) {
      ...
      try {
        ...
        console.log('Failed to parse JSON, trying to extract questions directly');
