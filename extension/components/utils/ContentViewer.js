...
function formatTables(tables) {
  return tables.map(table => {
    ...
    const formattedTable = {
      ...
      rows: table.rows || [],
      columnCount: table.headers ? table.headers.length : 
                   (table.rows && table.rows.length > 0 ? table.rows[0].length : 0)
    };
...
function detectColumnTypes(table) {
  ...
  for (let i = 0; i < columnCount; i++) {
    ...
    //  predominant type
    if (numericCount >= dateCount && numericCount >= booleanCount && numericCount > sampleSize / 2) {
      types.push('numeric');
    } else if (dateCount >= numericCount && dateCount >= booleanCount && dateCount > sampleSize / 2) {
      types.push('date');
    } else if (booleanCount >= numericCount && booleanCount >= dateCount && booleanCount > sampleSize / 2) {
      types.push('boolean');
    } else {
      types.push('text');
    }
    ...
    <div class="content-viewer">
      <div class="content-viewer-header">
        ...
        <div class="content-viewer-meta">
          <div><strong>URL:</strong> <a href="${data.metadata.url}" target="_blank">${escapeHtml(data.metadata.url)}</a></div>
          <div><strong>Extracted:</strong> ${new Date(data.metadata.timestamp).toLocaleString()}</div>
          <div><strong>Words:</strong> ${data.content.summary.wordCount} | <strong>Characters:</strong> ${data.content.summary.characterCount}</div>
        </div>
      </div>
  `;
  ...
  if (data.hasSelection) {
    html += `
      <div class="content-viewer-selection">
        <h4>Selected Text</h4>
        <div class="content-viewer-selection-text">${escapeHtml(data.selection)}</div>
      </div>
    `;
  }
    ...
    if (data.structuredData.hasJsonLd) {
      html += `<div class="content-viewer-jsonld">
        ...
        <div class="jsonld-details" style="display: none;">
          <pre>${escapeHtml(JSON.stringify(data.structuredData.jsonLd, null, 2))}</pre>
        </div>
      </div>`;
    }
    
    // API documentation
    if (data.structuredData.apiDocumentation && data.structuredData.apiDocumentation.length > 0) {
      html += `<div class="content-viewer-api">
        <h5>API Documentation</h5>
        <div class="api-endpoints">
          ${data.structuredData.apiDocumentation.map(api => `
...
function attachContentViewerEvents(container) {
  // toggle buttons
  container.querySelectorAll('.toggle-json-button, .toggle-raw-button').forEach(button => {
